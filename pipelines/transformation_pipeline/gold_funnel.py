...
def gold_funnel_analysis():
    ...
    session_outcomes=(
        ...
        .agg(F.max(F.when(F.col("event_type")=="purchase",1).otherwise(0)).alias("converted"))    
    )
    return (
        session_outcomes
        .groupBy("referrer")
        .agg(F.count("session_id").alias("total_sessions"),
             F.sum("converted").alias("total_conversions"),
    ).withColumn("conversion_rate",F.col("total_conversions")/F.col("total_sessions"))
    )
